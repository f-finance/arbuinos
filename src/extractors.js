...
const createExtractor = (dex) => {
  return async (state) => {
    const contractsPools = await Promise.all(
      Array.from(state).map(async ([contractAddress, contractStorage]) => {
        const pools = await contractStorageToPoolsExtractors[dex](
          contractStorage
        );

        return pools.map((pool) => {
          return { dex, contractAddress, ...pool };
        });
      })
    );
    return contractsPools.flat();
  };
};

/**
 * Extracts pool infos from dex contract states
 *
 * Returns array of objects. Each object has keys:
 *   dex (string) - name of dex
 *   contractAddress (string) - address of pool contract
 *   address1 (string) - first token identifier
 *   address2 (string) - second token identifier
 *   liquidity1 (BN) -
 *   liquidity2 (BN) -
 *   fee1 (BN) -
 *   fee2 (BN) -
 */
export const stateToPoolsExtractors = {
  [DEX.QUIPUSWAP]: createExtractor(DEX.QUIPUSWAP),
  [DEX.PLENTY]: createExtractor(DEX.PLENTY),
  [DEX.VORTEX]: createExtractor(DEX.VORTEX),
  // [DEX.FLAME]: createExtractor(DEX.FLAME),
  [DEX.TZBTCORIGINAL]: createExtractor(DEX.TZBTCORIGINAL),
  [DEX.SPICYSWAP]: createExtractor(DEX.SPICYSWAP),
};
